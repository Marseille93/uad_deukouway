...
export async function DELETE(
	...
	try {
		...
		if (
			listing.user_id !== decoded.userId &&
...
export async function PATCH(
	...
	try {
		...
		const { availableSpots } = await request.json();

		// Vérifier que le nombre de places est valide
		if (availableSpots < 0) {
			return NextResponse.json(
				{ error: "Le nombre de places ne peut pas être négatif" },
				{ status: 400 }
			);
		}

		// Mettre à jour le nombre de places et le statut
		const { error: updateError } = await supabaseAdmin
			.from("listings")
			.update({
				available_spots: availableSpots,
				status: availableSpots === 0 ? "inactive" : "active",
		...
		console.error("Erreur API update spots:", error);
		return NextResponse.json(
			{ error: "Erreur interne du serveur" },
			{ status: 500 }
		);
